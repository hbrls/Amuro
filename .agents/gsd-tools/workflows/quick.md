...
Step 1 — HEAD attachment assertion (MANDATORY, runs before any reset/commit):
  ...
  if ! echo "$ACTUAL_BRANCH" | grep -Eq '^worktree-agent-[A-Za-z0-9._/-]+$'; then
    echo "FATAL: worktree HEAD '$ACTUAL_BRANCH' is not in the worktree-agent-* namespace (Cline's per-agent worktree branch namespace)." >&2
...
1. **Worktree cleanup:** If the executor ran with `isolation="worktree"`, merge the worktree branch back and clean up:
   ...
   # `.cline/worktrees/agent-` (the namespace Cline's `isolation="worktree"`
